    ...
    */
    ...
    const tenantProfile = await prisma.tenant.upsert({
      ...
      create: {
        ...
        shopName: 'Jerick Hub',
        unitId: 'L1-105',
        description: 'Premium electronics and gadgets store with the latest tech products.',
        galleryUrls: [
      ...
      await Promise.all([
        prisma.review.create({
          data: {
            userId: customer.id,
            rating: 5,
            comment: 'Excellent shopping experience! Great stores and amazing atmosphere.',
            isApproved: true,
          },
        }),
        prisma.review.create({
          data: {
            userId: customer.id,
            rating: 4,
            comment: 'Good variety of shops, but could use more dining options.',
            isApproved: true,
          },
        }),
      ]);
