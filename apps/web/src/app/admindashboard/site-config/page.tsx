...
import { Layout, Type, Image as ImageIcon, ToggleLeft, Save, RefreshCcw, Monitor, Smartphone, CheckCircle, AlertTriangle, Eye } from 'lucide-react';
...
export default function SiteConfigCMS() {
  ...
  if (loading) return <div className="p-10 text-center animate-pulse uppercase tracking-[0.2em] font-bold text-xs text-slate-400">Initializing CMS Engine...</div>;

  return (
    <div className="p-8 lg:p-10 animate-fade-in-up max-w-[1600px] mx-auto space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-charcoal dark:text-white tracking-tight">Public-View CMS Control</h1>
          <p className="text-sm text-slate-500 font-medium mt-1">Manage branding, headlines, and system maintenance mode from a central dashboard.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <button onClick={() => setDraft(config)} className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-zinc-900 text-slate-400 dark:text-white border border-slate-100 dark:border-white/5 rounded-xl font-bold text-xs uppercase tracking-widest hover:border-primary transition-all">
          ...
          >
            {isSaving ? <><RefreshCcw size={14} className="animate-spin" /> Publishing...</> : <><Save size={14} /> Publish Changes</>}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        ...
        <div className="space-y-8 h-fit">
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 rounded-[2rem] shadow-sm p-8 space-y-8">
            <div className="flex items-center gap-3 pb-6 border-b border-slate-100 dark:border-white/5">
               <div className="p-2.5 bg-primary/10 rounded-xl text-primary"><Type size={18} /></div>
               <h3 className="font-black text-charcoal dark:text-white uppercase tracking-widest text-xs">Hero Section Content</h3>
            </div>

            <div className="space-y-6">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Main Headline</label>
                <textarea 
                  rows={2}
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Sub-headline Description</label>
                <textarea 
                   rows={3}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Button Text</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Background Overlay (Darkness)</label>
                  <input 
                    type="range" 
                    min="0" max="100"
                    value={draft.heroOverlayDark}
                    onChange={e => setDraft({...draft, heroOverlayDark: parseInt(e.target.value)})}
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 rounded-[2rem] shadow-sm p-8 space-y-6">
            <div className="flex items-center gap-3 pb-6 border-b border-slate-100 dark:border-white/5">
               <div className="p-2.5 bg-amber-500/10 rounded-xl text-amber-500"><ToggleLeft size={18} /></div>
               <h3 className="font-black text-charcoal dark:text-white uppercase tracking-widest text-xs">System & Maintenance</h3>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-zinc-800 rounded-2xl border border-slate-100 dark:border-white/5">
               <div className="flex items-start gap-3">
                  <div className="p-2 bg-white dark:bg-zinc-900 rounded-lg text-amber-500"><AlertTriangle size={18}/></div>
                  <div>
                    <h4 className="text-sm font-bold text-charcoal dark:text-white">Maintenance Mode</h4>
                    <p className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">Shut down public access for all visitors.</p>
                  </div>
               </div>
               <button 
                onClick={() => setDraft({...draft, isMaintenance: !draft.isMaintenance})}
                className={`w-14 h-8 rounded-full transition-colors relative ${draft.isMaintenance ? 'bg-primary' : 'bg-slate-300'}`}
               >
                 <div className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-md transition-transform ${draft.isMaintenance ? 'left-7' : 'left-1'}`} />
               </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 flex flex-col items-center">
           ...
           <div className={`relative transition-all duration-500 border border-slate-200 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl ${viewMode === 'desktop' ? 'aspect-video w-full' : 'w-[375px] h-[667px]'}`}>
             
             {/* Dynamic Content Engine (Mini-Clone of Public-View) */}
              <div 
                className="absolute inset-0 bg-cover bg-center transition-all duration-700"
                style={{ backgroundImage: `url(${draft.heroBgUrl || 'https://images.unsplash.com/photo-1519642918688-7e43b19245d8?auto=format&fit=crop&q=80&w=2000'})` }}
              >
                <div 
                ...
                <div className="absolute inset-x-0 bottom-0 top-0 flex flex-col items-center justify-center text-center px-10">
                   <div className="space-y-4 max-w-xl">
                      <span className="inline-block px-4 py-1.5 bg-primary/20 backdrop-blur-md text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-full border border-white/20 animate-fade-in shadow-xl">
                        {draft.isMaintenance ? '⚠️ SYSTEM OFFLINE' : 'SR-MANAGE CONTENT PREVIEW'}
                      </span>
              ...
              {draft.isMaintenance && (
                <div className="absolute inset-0 bg-charcoal flex flex-col items-center justify-center p-10 z-20">
                    ...
                    <h2 className="text-2xl font-black text-white mb-2 uppercase tracking-tight">System Under Maintenance</h2>
                    <p className="text-xs text-slate-400 font-medium text-center uppercase tracking-widest leading-loose">The public view will be hidden until you toggle this setting off and republish.</p>
                </div>
              )}
           ...
           <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 p-4 rounded-xl flex items-start gap-3 w-full max-w-lg">
               ...
               <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                 You are editing a draft. Changes will only take effect on the production Public-View once you click the <span className="text-primary font-black">Publish Changes</span> button.
               </p>
            </div>
        </div>
