  ...
  try {
    ...
    if (provider === 'cloudinary') {
      ...
      if (!cloudName || !apiKey || !apiSecret) {
        return NextResponse.json({ error: 'Cloudinary credentials missing in server environment' }, { status: 500 });
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (provider === 'cloudinary') {
      ...
      if (!cloudName || !apiKey || !apiSecret) {
        return NextResponse.json({ success: true, message: 'Cloudinary record detached (missing API keys to destroy)' });
      }
      ...
      return NextResponse.json({ success: true, message: 'Cloudinary record detached and destroyed' });
    }
