    ...
    async function loadData() {
      ...
      if (result.success && result.data) {
        ...
        const dist = [5, 4, 3, 2, 1].map(s => ({
          stars: s,
          pct: result.data.length > 0 ? Math.round((counts[s] / result.data.length) * 100) : 0
        }));
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
           <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/10 rounded-xl text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-300 shadow-sm">
             <Star size={14} className="text-amber-500 fill-amber-500 sm:w-4 sm:h-4" /> {isLoading ? '...' : avgRating} / 5.0
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 rounded-2xl sm:rounded-[2rem] shadow-sm p-4 sm:p-6 text-center">
             ...
             <p className="text-3xl sm:text-4xl lg:text-5xl font-black text-charcoal dark:text-white mb-2">{isLoading ? '...' : avgRating}</p>
             <div className="flex justify-center gap-1 text-amber-500 mb-2">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={14} className={clsx(i < Math.round(avgRating) ? 'fill-amber-500' : 'fill-slate-200 text-slate-200', 'sm:w-[18px] sm:h-[18px]')} />
                ))}
             </div>
             <p className="text-xs sm:text-sm font-medium text-slate-500">Based on {isLoading ? '...' : reviews.length} verified ratings</p>
          </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-white/5">
              {isLoading ? (
                <div className="p-10 text-center">
                  ...
                  <p className="mt-4 text-sm text-slate-500">Loading verified reviews...</p>
                </div>
              ) : reviews.length > 0 ? (
                  ...
                  <div key={review.id} className="p-4 sm:p-6 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                     <div className="flex items-start justify-between mb-3">
                       <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <div>
                            <h4 className="font-bold text-charcoal dark:text-white text-xs sm:text-sm">{review.user?.name || 'Anonymous'}</h4>
                            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{new Date(review.createdAt).toLocaleDateString()}</span>
                          </div>
                       </div>
                       <div className="flex items-center gap-2 sm:gap-4">
                         ...
                         <span className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-green-50 dark:bg-green-900/30 text-green-600 text-[8px] sm:text-[9px] font-bold uppercase tracking-widest">
                           <CheckCircle size={10} className="sm:w-3 sm:h-3" /> Published
                         </span>
                       </div>
                     </div>
                     <p className="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed ml-10 sm:ml-14">&ldquo;{review.comment || 'No comment provided.'}&rdquo;</p>
                  </div>
                ))
              ) : (
                <div className="p-20 text-center">
                   ...
                   <h3 className="text-lg font-bold text-charcoal dark:text-white mb-2">No Reviews Yet</h3>
                   <p className="text-sm text-slate-500 max-w-xs mx-auto">Customer feedback will appear here once approved by mall administration.</p>
                </div>
              )}
            </div>
